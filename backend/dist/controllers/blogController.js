...
const getAllBlogs = async (req, res) => {
    try {
        const [blogs] = await db_1.default.query(`SELECT b.blog_id, b.title, b.content, b.image, b.created_at, a.username, a.profile_image
       FROM blog b
...
const getBlogById = async (req, res) => {
    ...
    try {
        const [blogs] = await db_1.default.query(`SELECT b.blog_id, b.title, b.content, b.image, b.created_at, b.updated_at,
        a.username, a.profile_image, a.account_id as author_id
       FROM blog b
       JOIN account a ON b.author_id = a.account_id
       WHERE b.blog_id = ? AND b.status = 'published'`, [blogId]);
        if (blogs.length === 0) {
        ...
        const [relatedBlogs] = await db_1.default.query(`SELECT b.blog_id, b.title, b.image, b.created_at
       FROM blog b
       WHERE b.blog_id != ? AND b.status = 'published'
       ORDER BY CASE WHEN b.author_id = ? THEN 0 ELSE 1 END, b.created_at DESC
       LIMIT 4`, [blogId, blog.author_id]);
