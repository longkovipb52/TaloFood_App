...
exports.exportReportData = exports.getCustomerStats = exports.getProductStats = exports.getRevenueStats = exports.getOverviewStats = void 0;
...
const getOverviewStats = async (req, res) => {
    try {
        ...
        // 1. Tổng doanh thu
        const [revenue] = await db_1.default.query(`SELECT SUM(total_amount) as total FROM bill WHERE ${timeFilter}`);
        // 2. Tổng số đơn hàng
        const [orders] = await db_1.default.query(`SELECT 
        COUNT(*) as total,
      ...
      FROM bill WHERE ${timeFilter}`);
        ...
        const [newUsers] = await db_1.default.query(`SELECT COUNT(*) as total FROM account WHERE id_role = 2`);
        // 4. Tổng số sản phẩm đã bán
        const [itemsSold] = await db_1.default.query(`SELECT SUM(count) as total 
       FROM bill_info bi 
       JOIN bill b ON bi.id_bill = b.bill_id 
       WHERE ${timeFilter.replace('created_at', 'b.created_at')}`);
        // 5. Đánh giá trung bình
        const [rating] = await db_1.default.query(`SELECT AVG(star_rating) as average FROM reviews WHERE ${timeFilter}`);
        res.json({
...
const getRevenueStats = async (req, res) => {
    try {
        const period = req.query.period || 'month'; // 'day', 'week', 'month', 'year'
        const limit = parseInt(req.query.limit) || 12; // Số điểm dữ liệu muốn lấy
        let groupBy;
        let timeFilter;
        let labelFormat;
        switch (period) {
        ...
        const [revenue] = await db_1.default.query(`SELECT 
        ${groupBy} as date,
      ...
      LIMIT ?`, [limit]);
        ...
        const [paymentMethods] = await db_1.default.query(`SELECT 
        payment_method,
        SUM(total_amount) as amount,
        COUNT(*) as count
      FROM bill
...
const getProductStats = async (req, res) => {
    try {
      ...
      LIMIT ?`;
        ...
        const [topProducts] = await db_1.default.query(query, queryParams);
        ...
        const [categoryStats] = await db_1.default.query(`SELECT 
        fc.foodcategory_id,
...
const getCustomerStats = async (req, res) => {
    try {
        ...
        const [topCustomers] = await db_1.default.query(`SELECT 
      ...
      LIMIT 10`);
        ...
        // 2. Thống kê đánh giá
        const [ratingStats] = await db_1.default.query(`SELECT 
        star_rating,
        COUNT(*) as count
      FROM reviews
      ...
      ORDER BY star_rating DESC`);
        ...
        const [orderTimeStats] = await db_1.default.query(`SELECT 
    ...
    try {
        ...
        switch (reportType) {
            case 'revenue': {
                ...
                const [dailyRevenue] = await db_1.default.query(`SELECT 
          ...
          ORDER BY date`);
                // Lấy doanh thu theo phương thức thanh toán
                const [paymentStats] = await db_1.default.query(`SELECT 
            payment_method,
            COUNT(*) as order_count,
            SUM(total_amount) as revenue
          FROM bill
          ...
          GROUP BY payment_method`);
            ...
            case 'products': {
                ...
                const [topProducts] = await db_1.default.query(`SELECT 
