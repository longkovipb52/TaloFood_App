...
export const createOrder = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const [billResult]: any = await pool.query(
      'INSERT INTO bill (ngaydat, ngaygiao, id_account, status, address, total_amount, payment_method, phone, name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        currentDate,
    ...
    for (const item of items) {
      await pool.query(
        'INSERT INTO bill_info (id_bill, id_food, id_account, count, price) VALUES (?, ?, ?, ?, ?)',
        [billId, item.foodId, userId, item.quantity, item.price]
      );
      
      // Cập nhật số lượng đã bán (total_sold) của món ăn
      await pool.query(
        'UPDATE food SET total_sold = total_sold + ? WHERE food_id = ?',
...
export const cancelOrder = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const [orderInfo]: any = await pool.query(
      'SELECT * FROM bill WHERE bill_id = ? AND id_account = ?',
      [orderId, userId]
    );
