...
export const checkAdminRole = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const [rows]: any = await pool.query(
      'SELECT id_role FROM account WHERE account_id = ?',
      [userId]
    );
...
export const getDashboardStats = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [usersCount]: any = await pool.query(
      'SELECT COUNT(*) as count FROM account WHERE id_role = 2'
    );
    
    // Tổng số sản phẩm
    const [productsCount]: any = await pool.query(
    ...
    const [revenue]: any = await pool.query(
      'SELECT SUM(total_amount) as total FROM bill WHERE status = "Đã giao" OR status = "Đã thanh toán"'
    );
    
    // Số đơn hàng theo trạng thái
    const [orderStatus]: any = await pool.query(
      `SELECT 
    ...
    const [recentOrders]: any = await pool.query(
      `SELECT b.bill_id, b.name, b.status, b.total_amount, b.created_at
       FROM bill b
