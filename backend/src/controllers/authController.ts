...
export const login = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const [rows]: any = await pool.query(
      'SELECT account_id, username, password, id_role FROM account WHERE username = ?',
      [username]
    );
    ...
    const token = jwt.sign(
      { id: user.account_id, username: user.username, role: user.id_role },
      'your_jwt_secret',
      { expiresIn: '1d' }
    );
...
export const register = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // Thêm user mới
    await pool.query(
      'INSERT INTO account (username, password, email, phone, address, id_role, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [username, hashedPassword, email, phone, address, 2, 1]
    );
